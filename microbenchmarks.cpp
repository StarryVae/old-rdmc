...
uint32_t ping_pong() {
    ...
    //    psm_mq_irecv(mq, 1, 1, 0, new char[1024*1024], 1024*1024, NULL,
    // &req2);
    ...
    if(node_rank % 2 == 0) {
        ...
        for(int i = 0; i < NUM_BOUNCES; i++) {
            if(i == WARM_UP - 1) t = get_time();

            psm_mq_send(mq, epaddrs[(node_rank ^ 1)], PSM_MQ_FLAG_SENDSYNC, tag,
                        buffer, 4);
            //            psm_mq_isend(mq, epaddrs[(node_rank ^ 1)],
            //                         PSM_MQ_FLAG_SENDSYNC, 0, buffer, 4, NULL,
            // &req);
...
uint32_t latency_test() {
    ...
    if(node_rank % 2 == 0) {
        ...
        psm_mq_send(mq, epaddrs[(node_rank ^ 1)], PSM_MQ_FLAG_SENDSYNC, tag + 2,
                    buffer, 4);
        ...
        psm_mq_send(mq, epaddrs[(node_rank ^ 1)], PSM_MQ_FLAG_SENDSYNC, tag + 1,
                    buffer2, 4);
    }
    return n;
}
...
void wait() {
    psm_mq_req_t req;
    while(psm_mq_ipeek(mq, &req, NULL) != PSM_OK) /* do nothing */
        ;
...
void sbandwidth_test() {
    for(size_t num = 1; num <= 1024; num *= 2) {
        ...
        for(size_t total = 16 << 20; total >= 16 << 10 && total >= num * 4096;
            total /= 2) {
...
//#define SBANDWIDTH_USE_MMAP
            ...
            char *buffer =
                (char *)mmap(NULL, size * num, PROT_READ | PROT_WRITE,
            ...
            for(int iteration = 0; iteration < 32; iteration++) {
                ...
                if(node_rank % 2 == 0) {
                    // Wait for an initial send, so we know that the other node
                    // is ready.
                    psm_mq_irecv(mq, 0, 0, 0, buffer, 4, NULL, &req);
                    psm_mq_wait(&req, NULL);

                    // Time returns the number of nanoseconds since some fixed
                    // start time.
                    uint64_t t = get_time();

                    // Start concurrent send and receive.
                    for(unsigned int i = 0; i < num; i++) {
                        psm_mq_irecv(mq, i, ~0ull, 0, buffer + size * i, size,
                                     NULL, &req);
                        psm_mq_isend(mq, epaddrs[node_rank + 1],
                                     PSM_MQ_FLAG_SENDSYNC, i,
                                     buffer2 + size * i, size, NULL, &req);
                    }
                    // Wait for them to complete.
                    for(unsigned int i = 0; i < num * 2; i++) wait();

                    // Compute how long these sends took
                    uint64_t diff = get_time() - t;
                    rates.push_back(8.0 * total / diff);
                } else {
                    // Post an initial send, and wait for it to complete before
                    // we
                    // initiate
...
void sbandwidth_test2() {
    ...
    for(size_t size = total; size >= 4096; size /= 2) {
        ...
        if(node_rank % 2 == 0) {
            // Wait for an initial send, so we know that the other node is
            // ready.
            psm_mq_irecv(mq, 0, 0, 0, buffer, 4, NULL, &req);
            psm_mq_wait(&req, NULL);

            // Time returns the number of nanoseconds since some fixed start
            // time.
            uint64_t t = get_time();
            ...
            // Post an initial send, and wait for it to complete before we
            // initiate
            // the concurrent send and receive.
            psm_mq_isend(mq, epaddrs[node_rank - 1], PSM_MQ_FLAG_SENDSYNC, 0,
                         buffer, 4, NULL, &req);
...
void bandwidth_test() {
    ...
    char *buffer =  //(char*)malloc(size);
        (char *)mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE,
                     -1, 0);
    ...
    if(node_rank == 0) {
        ...
        for(int i = 1; i < num_nodes; i++) {
            ...
            psm_mq_isend(mq, epaddrs[i], PSM_MQ_FLAG_SENDSYNC, 0, buffer2, size,
                         NULL, &req);
        ...
        psm_mq_isend(mq, epaddrs[0], PSM_MQ_FLAG_SENDSYNC, node_rank, buffer2,
                     4, NULL, &req);
        psm_mq_wait(&req, NULL);
...
void memcpy_test() {
    ...
    for(size_t i = 0; i < size; i++) buf2[i] = 0;  // buf1[i+1024];
...
void memtest2() {
    for(size_t size = 4096; size < (1 << 30); size *= 2) {
        ...
        mremap(buffer2, size, size, MREMAP_FIXED | MREMAP_MAYMOVE, buffer);
